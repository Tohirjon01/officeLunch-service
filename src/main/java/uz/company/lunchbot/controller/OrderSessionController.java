...
public class OrderSessionController {
    ...
    public ApiResponse<SessionRecalculationResponse> recalculate(@PathVariable Long id,
                                                                 @Valid @RequestBody RecalculateSessionRequest request,
                                                                 @RequestHeader("X-Actor-User-Id") Long actorUserId) {
        return ApiResponse.ok(orderSessionService.recalculateSession(id, request, actorUserId));
    }
}
